...
void RenderDevice::create_swapchain() {
	...
	vk::SwapchainCreateInfoKHR create_info(
		...
		indices.graphics.value() == indices.present.value() ? vk::SharingMode::eExclusive
...
u32 RenderDevice::new_frame() {
	...
	VkResult result = vkAcquireNextImageKHR(m_device, m_swapchain, UINT64_MAX, m_image_available_semaphores[m_frame],
											nullptr,
											&m_swapchain_image);
...
Handle<Shader> RenderDevice::create_shader(const ShaderDef &def) {
	...
	vk::PipelineShaderStageCreateInfo vertex_shader_stage_info({}, vk::ShaderStageFlagBits::eVertex, vertex_module,
															   "main");
	vk::PipelineShaderStageCreateInfo fragment_shader_stage_info({}, vk::ShaderStageFlagBits::eFragment,
																 fragment_module, "main");
...
static void transition_image_layout(vk::CommandBuffer cmd, vk::Image image, vk::Format format, vk::ImageLayout old_layout, vk::ImageLayout new_layout) {
	...
	if (old_layout == vk::ImageLayout::eUndefined && new_layout == vk::ImageLayout::eTransferDstOptimal) {
		...
		EMBER_ASSERT(false); // unsupported layout transition
	}

	cmd.pipelineBarrier(src_stage, dst_stage, {}, {}, {}, barrier);
}

Handle<Texture> RenderDevice::create_texture(const TextureDef &def) {
	auto handle = m_textures.emplace();
	auto texture = m_textures.get(handle);

	VkImageCreateInfo image_info = { VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO };
	image_info.format = (VkFormat) to_vk_image_format(def.format);
	image_info.imageType = (VkImageType) to_vk_image_type(def.type);
	image_info.extent.width = def.width;
	image_info.extent.height = def.height;
	image_info.extent.depth = def.depth;
	...
	const bool is_render_target = (def.flags & TextureFlags::RenderTarget) == TextureFlags::RenderTarget;
	...
	VK_CHECK(vmaCreateImage(m_vma, &image_info, &memory_info,
							&temp_image, &texture->allocation, nullptr));

	texture->image = vk::Image(temp_image);

	// Create the image view
	vk::ImageViewCreateInfo view_info(
		{},
		texture->image,
		to_vk_view_type(def.type),
		to_vk_image_format(def.format),
		{},
		vk::ImageSubresourceRange(vk::ImageAspectFlagBits::eColor, 0, 1, {}, 1)
	);
	texture->view = m_device.createImageView(view_info);
	texture->layout = vk::ImageLayout::eUndefined;

	// if initial data was provided, use copy buffer to upload it to the image
	if (def.data) {
		// create staging buffer
		auto buf_handle = create_buffer({
			.usage = BufferUsage::Staging,
			.memory = MemoryType::Shared,
			.data = def.data,
			.size = (u64) def.width * def.height * 4,
		});

		auto buf = get_buffer(buf_handle);
		...
		vk::BufferImageCopy region(
			...
			vk::Offset3D(0, 0, 0),
			vk::Extent3D(def.width, def.height, def.depth)
		);

		// transition the image to receive the data from the buffer
		transition_image_layout(cmd->m_cmd, texture->image, texture->format, vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal);
		// copy the data to the image
		cmd->m_cmd.copyBufferToImage(buf->buffer, texture->image, vk::ImageLayout::eTransferDstOptimal, region);
...
Texture *RenderDevice::get_texture(Handle<Texture> handle) {

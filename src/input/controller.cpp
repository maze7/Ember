...
bool Controller::down(Button button) const {
...
bool Controller::pressed(Button button) const {
	u32 index = (u32) button;
...
bool Controller::released(Button button) const {
	u32 index = (u32) button;
...
void Controller::rumble(float low_intensity, float high_intensity, float duration) {
	...
	u16 high_freq = Math::clamp(high_intensity, 0, 1) * 0xFFFF;
	u16 low_freq = Math::clamp(low_intensity, 0, 1) * 0xFFFF;
...
void Controller::on_button(Button button, bool down) {
	u32 index = (u32) button;

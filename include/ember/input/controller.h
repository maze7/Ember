...
namespace Ember
{
	...
	public:
		...
		/**
		 * @return controller instance_id, this does not change while controller is connected.
		 */
		[[nodiscard]] u32 id() const { return m_id; }

		/**
		 * @return controller index within controllers array, this does not change while controller is connected.
		 */
		...
		/**
		 * Returns whether a given controller button is currently being pressed down.
		 *
		 * @param button Controller button that is being checked
		 * @return boolean state of given controller button
		 */
		[[nodiscard]] bool down(Button button) const;

		/**
		 * Returns whether a given controller button was pressed in the current frame.
		 *
		 * @param button Controller button that is being checked
		...
		/**
		 * Returns whether a given controller button was released in the current frame.
		 *
		 * @param button Controller button that is being checked
		 * @return boolean state of the given controller button
		 */
		...
		/**
		 * Rumbles the controller for a given duration. This will cancel any previous rumble effects.
		 * @param low_intensity From 0.0 to 1.0 intensity of the Low-intensity rumble
		 * @param high_intensity From 0.0 to 1.0 intensity of the High-intensity rumble
		 * @param duration how long, in seconds, for the rumble to last.
		 */
	...
	private:
		 ...
		 * @param is_gamepad boolean declaring whether the controller is a gamepad or joystick
		...
		/**
		 * Called by the `Window` when an underlying SDL3 controller is disconnected
		 */
		void disconnect();
		...
		/// controller index within the controllers array, this will not change while the controller is connected
		u32 m_index = 0;
